...
class exportObj.SquadBuilder
    ...
    onSquadDirtinessChanged: () =>
        ...
        if @ships.length > 1
            $('meta[property="og:description"]').attr("content", @uitranslation("X-Wing Squadron by YASB: ") + @current_squad.name + ": ")
        else
    ...
    showChooseObstaclesSelectSources: (obstacle) ->
        ...
        @obstacles_select_sources.text if (sources.length > 1) or (not (exportObj.translate('sources', 'Loose Ships') in sources)) then (if sources.length > 0 then sources.join(', ') else "") else @uitranslation("Only available from 1st edition")
    ...
    serialize: ->
        ...
        """v#{serialization_version}ZhZ#{selected_points}Z#{( ship.toSerialized() for ship in @ships when ship.pilot? and (not @isQuickbuild or ship.primary) ).join 'Y'}"""
    ...
    showTooltip: (type, data, additional_opts, container = @info_container, force_update = false) ->
        if data != @tooltip_currently_displaying or force_update
            ...
            # we get all pilots for the ship, to display stuff like available slots which are treated as pilot properties, not ship properties (which makes sense, as they depend on the pilot, e.g. talent or force slots)
                    ...
                    container.find('.info-name').html """#{if data.display_name then data.display_name else data.name}"""
                    ...
                    container.find('.info-sources.info-data').text if (sources.length > 1) or (not (exportObj.translate('sources', 'Loose Ships') in sources)) then (if sources.length > 0 then sources.join(', ') else "") else @uitranslation("Only available from 1st edition")
                ...
                when 'Pilot'
                    ...
                    container.find('.info-name').html """#{uniquedots}#{if data.display_name then data.display_name else data.name}#{if exportObj.isReleased(data) then "" else ""}"""
...
#                        container.find('tr.info-attack td.info-header i.xwing-miniatures-font').removeClass(cls) if cls.startsWith('xwing-miniatures-font-attack')
                ...
                when 'Quickbuild'
                    ...
                    container.find('.info-name').html """#{uniquedots}#{if pilot.display_name then pilot.display_name else pilot.name}#{if data.suffix? then data.suffix else ""}"""
...
#                        container.find('tr.info-attack td.info-header i.xwing-miniatures-font').removeClass(cls) if cls.startsWith('xwing-miniatures-font-frontarc')
                ...
                when 'Addon'
                    ...
                    if @collection?.counts?
                        ...
                        container.find('.info-collection').hide()
                    container.find('.info-name').html """#{uniquedots}#{if data.display_name then data.display_name else data.name}"""
